...
class TestNode():
    ...
    def __init__(self, i, datadir, *, host, rpc_port, p2p_port, timewait, bitcoind,
        ...
        self.bitcoinconf = os.path.join(self.datadir, "zeniq.conf")
        ...
        if not os.path.isfile(self.binary):
            raise FileNotFoundError(
                "Binary '{}' could not be found.\nTry setting it manually:\n\tBITCOIND=<path/to/zeniqd> {}".format(self.binary, sys.argv[0]))
        ...
        if use_cli and not os.path.isfile(bitcoin_cli):
            raise FileNotFoundError(
                "Binary '{}' could not be found.\nTry setting it manually:\n\tBITCOINCLI=<path/to/zeniq-cli> {}".format(bitcoin_cli, sys.argv[0]))
    ...
    def start(self, extra_args=None, stdout=None,
        ...
        self.running = True
        self.log.debug("zeniqd started, waiting for RPC to come up")
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                raise FailedToStartError(self._node_msg(
                    'zeniqd exited with status {} during initialization'.format(self.process.returncode)))
            try:
        ...
        self._raise_assertion_error("Unable to connect to zeniqd")
    ...
    def assert_start_raises_init_error(
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug('zeniqd failed to start: {}'.format(e))
                self.running = False
                self.process = None
                ...
                if expected_msg is None:
                    assert_msg = "zeniqd should have exited with an error"
                else:
                    assert_msg = "zeniqd should have exited with expected error " + expected_msg
    ...
    def calculate_fee(self, tx):
        ...
        # relay_fee gives a value in ZENIQ per kB.
...
class TestNodeCLI():
    """Interface to zeniq-cli for an individual node"""
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run zeniq-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value)
                      for (key, value) in kwargs.items()]
        assert not (
            pos_args and named_args), "Cannot use positional arguments and named arguments in the same zeniq-cli call"
        ...
        if self.emulator is not None:
            p_args = [self.emulator] + p_args
        self.log.debug("Running zeniq-cli command: {}".format(command))
