...
def assert_fee_amount(fee, tx_size, fee_per_kB, wiggleroom=2):
    ...
    if fee < (tx_size - wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} ZENIQ too low! (Should be {} ZENIQ)".format(str(fee), str(target_fee)))
    if fee > (tx_size + wiggleroom) * fee_per_kB / 1000:
        raise AssertionError(
            "Fee of {} ZENIQ too high! (Should be {} ZENIQ)".format(str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting ZENIQ values"""
...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "zeniq.conf"), 'w', encoding='utf8') as f:
        f.write("regtest=1\n")
        f.write("[regtest]\n")
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "zeniq.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "zeniq.conf")):
        with open(os.path.join(datadir, "zeniq.conf"), 'r', encoding='utf8') as f:
            for line in f:
