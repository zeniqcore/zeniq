...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave zeniqd's and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop zeniqd's after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use zeniq-cli instead of RPC for all commands")
        ...
        parser.add_argument("--extra-bitcoind-args", dest="extra_bitcoind_args", default="",
                            help="Start zeniqd with these additional arguments (comma separated)")
        ...
        self.options.bitcoind = os.getenv(
            "BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/zeniqd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv(
            "BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/zeniq-cli' + config["environment"]["EXEEXT"])
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            # Overwrite port/rpcport in zeniq.conf
    ...
    def skip_if_no_cli(self):
        """Skip the running test if zeniq-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("zeniq-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether zeniq-cli was compiled."""
        config = configparser.ConfigParser()
        config.read_file(open(self.options.configfile, encoding='utf-8'))
