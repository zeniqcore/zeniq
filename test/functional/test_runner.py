...
def main():
    ...
    parser.add_argument('--testsuitename', '-n', default='Zeniq functional tests',
...
def run_tests(test_list, build_dir, tests_dir, junitoutput, tmpdir, num_jobs, test_suite_name,
    ...
    try:
        pidofOutput = subprocess.check_output(["pidof", "zeniqd"])
        if pidofOutput is not None and pidofOutput != b'':
            print("{}WARNING!{} There is already a zeniqd process running on this system. Tests may fail unexpectedly due to resource contention!".format(
                BOLD[1], BOLD[0]))
