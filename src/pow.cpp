...
static uint32_t GetNextASERTWorkRequired(const CBlockIndex *pindexPrev,
    ...
    arith_uint256 nextTarget = CalculateASERT(refBlockTarget,
                                              ...
                                              nHeightDiff,
                                              powLimit,
          params.nASERTHalfLife[pindexPrev->nHeight>=params.ZeniqASERTActive]);//12h then 3h
...
arith_uint256 CalculateASERT(const arith_uint256 &refTarget,
    ...
    // overflows easily. 24 leading zero bits is more than enough.
    assert((powLimit >> 232) == 0);
...
uint32_t GetNextWorkRequired(const CBlockIndex *pindexPrev,
    ...
    bool axionenabled = (params.IsZenitNet()
            && params.asertAnchorParams) // this is for pow_tests
        ? pindexPrev->nHeight >= params.ZeniqASERTIntro
        : IsAxionEnabled(params,pindexPrev);
    ...
    if (params.IsZenitNet()) {
        if (IsDarkGravityWaveEnabled(params, pindexPrev)){
            return DarkGravityWave(pindexPrev, pblock, params);
        }
        // Just static blocks at this point in time
        const arith_uint256 powLimit = UintToArith256(params.powLimit);
