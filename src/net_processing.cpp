...
void AddToRelayMap(const CTransactionRef& tx) {
    AssertLockHeld(cs_main);
    mapRelay.insert(std::make_pair(tx->GetId(),tx));
}
...
static bool ProcessMessage(const Config &config, CNode *pfrom,
    ...
    if (strCommand == NetMsgType::VERACK) {
        ...
        if (g_Minter && 
            config.GetChainParams().GetConsensus().IsZenitNet() &&
    ...
    if (strCommand == NetMsgType::TX) {
        ...
        if (isDMA && g_Minter && g_Minter->TxSkip(tx)) {
            return true;
        }

        if (isDMA && AlreadyHave(inv)) {
            // re-relay if not known by peer
            RelayTransaction(tx, connman);
        } else if (isDMA && AcceptToMemoryPool(
                    config, g_mempool, state, ptx, &fMissingInputs,
                    true /* bypass limits */,
                    Amount::zero() /* nAbsurdFee */)) {
            g_mempool.check(pcoinsTip.get());
            RelayTransaction(tx, connman);
            pfrom->nLastTXTime = GetTime();
            // LogPrintf("Peer(%lld): DMA-tx %d %s\n", pfrom->GetId(), tx.nLockTime, tx.GetId().ToString());
        } else if (!AlreadyHave(inv) &&
