 ...
 *
 * This is the developer documentation of Zeniq Node
 * (https://www.zeniq.com/). Zeniq Node is a client for the
 * digital currency called Zeniq (https://zeniq.net/), which
 * enables instant payments to anyone, anywhere in the world. Zeniq uses
...
static bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/zeniq.conf are parsed in qt/bitcoin.cpp's
    // main()
    SetupServerArgs();
    std::string error;
    ...
    try {
        ...
        // Check for -regtest parameter (Params() calls are only
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr,
                        ...
                        "see zeniqd -h for a list of options.\n",
                        argv[i]);
                return false;
            }
        }

        // -server defaults to true for zeniqd but not for the GUI so do this
        // here
...
int main(int argc, char *argv[]) {
    ...
    // Connect zeniqd signal handlers
    noui_connect();
