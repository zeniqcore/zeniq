...
const char *BIP71_MIMETYPE_PAYMENT = "application/zeniq-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/zeniq-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST =
    "application/zeniq-paymentrequest";
#endif
...
static QString ipcServerName() {
    QString name("ZeniqQt");

    // Append a simple hash of the datadir
    // Note that GetDataDir(true) returns a different path for net
...
void PaymentServer::ipcParseCommandLine(interfaces::Node &node, int argc,
                                        char *argv[]) {
    std::array<const std::string *, 2> networks = {
        {&CBaseChainParams::ZENIQ, 
...
// clang-format on
{
    ...
    // on Mac: sent when you click zeniq: links
...
//
// OSX-specific way of handling zeniq: URIs and PaymentRequest mime types.
    ...
    if (GUIUtil::parseBitcoinURI(scheme, s, &recipient)) {
        ...
        Q_EMIT message(
            ...
            tr("URI cannot be parsed! This can be caused by an invalid "
               "Zeniq address or malformed URI parameters."),
            CClientUIInterface::ICON_WARNING);
    }
...
void PaymentServer::initNetManager() {
    ...
    // netManager is used to fetch paymentrequests given in Zeniq: URIs
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus &request,
    ...
    for (const std::pair<CScript, Amount> &sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(
                QString::fromStdString(EncodeLegacyAddr(dest, Params())));
        } else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom Zeniq addresses are
        ...
        // Zeniq amounts are stored as (optional) uint64 in the protobuf
