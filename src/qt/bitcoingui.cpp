...
void BitcoinGUI::createActions() {
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Zeniq address"));
    ...
    signMessageAction->setStatusTip(
        tr("Sign messages with your Zeniq addresses to prove you own them"));
    verifyMessageAction =
        new QAction(platformStyle->TextColorIcon(":/icons/verify"),
                    tr("&Verify Message..."), this);
    verifyMessageAction->setStatusTip(
        tr("Verify messages to ensure they were signed with specified Zeniq addresses"));
...
void BitcoinGUI::updateNetworkState() {
    ...
    if (clientModel->node().getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Zeniq network", "", count) +
