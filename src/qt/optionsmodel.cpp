...
void OptionsModel::Init(bool resetSettings) {
    ...
    // Display
    constexpr auto defaultDisplayUnit = BitcoinUnits::ZENIQ;
    ...
    if (!BitcoinUnits::valid(nDisplayUnit)) {
        ...
        // a unit we don't know about, so just default back to ZENIQ.
...
// Wallet
#ifdef ENABLE_WALLET
    settings.setValue("fAllowLegacyP2SH", gArgs.GetBoolArg("-allowlegacyp2sh", true));
    ...
    if (!settings.contains("fAllowLegacyP2PKH")) {
        settings.setValue("fAllowLegacyP2PKH", true); //Zeniq: no CashAddr
    }
