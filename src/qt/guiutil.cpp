...
QString convertToCashAddr(const CChainParams &params, const QString &addr) {
    ...
    return QString::fromStdString(EncodeLegacyAddr(dst, params));
}
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent) {
    ...
    widget->setPlaceholderText(
        QObject::tr("Enter a Zeniq address (e.g. %1)")
            .arg(QString::fromStdString(DummyAddress(GetConfig().GetChainParams()))));
    widget->setValidator(
        new AddressEntryValidator(Params().CashAddrPrefix(), parent));
...
bool parseBitcoinURI(const QString &scheme, const QUrl &uri,
    ...
    for (auto [key, value] : uriQuery.queryItems()) {
        ...
        if (key == "label") {
            ...
            if (!value.isEmpty()) {
                const auto amount = BitcoinUnits::parse(BitcoinUnits::ZENIQ, false, value);
                if (!amount) {
                    return false;
                }
                rv.amount = *amount;
...
bool parseBitcoinURI(const QString &scheme, QString uri,
                     SendCoinsRecipient *out) {
    //
    //    Cannot handle this later, because zeniq://
...
QString formatBitcoinURI(const CChainParams &params,
    ...
    if (info.amount != Amount::zero()) {
        ret +=
            QString("?amount=%1")
                .arg(BitcoinUnits::format(BitcoinUnits::ZENIQ, info.amount, false,
...
bool openBitcoinConf() {
    ...
    /* Open zeniq.conf with the associated application */
...
static fs::path StartupShortcutPath() {
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zeniq Node.lnk";
}
    ...
    if (fAutoStart) {
        ...
        if (SUCCEEDED(hres)) {
            ...
            strArgs += QString::fromStdString(strprintf(
                " -regtest=%d", gArgs.GetBoolArg("-regtest", false)));
...
static fs::path GetAutostartFilePath() {
    ...
    if (chain == CBaseChainParams::ZENIQ) {
        return GetAutostartDir() / "zeniq.desktop";
    }
    return GetAutostartDir() / strprintf("zeniq-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart) {
    if (!fAutoStart) {
        ...
        if (chain == CBaseChainParams::ZENIQ) {
            optionFile << "Name=Zeniq\n";
        } else {
            optionFile << strprintf("Name=Bitcoin (%s)\n", chain);
        }
        optionFile << "Exec=" << pszExePath
                   << strprintf(" -min -regtest=%d\n",
