...
TxId SendCoins(CWallet &wallet, SendCoinsDialog &sendCoinsDialog,
               Amount amount, bool legacyAddress=true, bool p2shAddress=false) {
...
void TestGUI() {
    ...
    QCOMPARE(transactionTableModel->rowCount({}), 106);
    TxId txid1 = SendCoins(*wallet.get(), sendCoinsDialog, 1 * COIN);
    TxId txid2 = SendCoins(*wallet.get(), sendCoinsDialog, 1 * COIN);
        ...
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(paymentTextList.at(3),
                     QString("Amount: 0.00000001 ") +
                         QString::fromStdString("ZENIQ"));
                         //not CURRENCY_UNIT, because separately hardcoded in qt
    ...
    QCOMPARE(transactionTableModel->rowCount({}), 110);
