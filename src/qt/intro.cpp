...
Intro::Intro(QWidget *parent, uint64_t blockchain_size,
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Zeniq block chain.")
            .arg(PACKAGE_NAME) +
...
QString Intro::getDefaultDataDirectory() {
    QString ret = GUIUtil::boostPathToQString(GetDefaultDataDir());
    ret.replace(".bitcoin",".zeniq");
    return ret;
}
...
bool Intro::pickDataDirectory(interfaces::Node &node) {
     ...
     * to
     * override -datadir in the zeniq.conf file in the default data directory
