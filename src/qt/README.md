...
See build instructions ([macOS](/doc/build-osx.md), [Unix](/doc/build-unix.md), etc).
...
Used to process BIP21 and BIP70 payment URI / requests. Also handles URI based application switching (e.g. when following a zeniq:... link from a browser).
...
* `bitcoinunits.(h/cpp)`: ZENIQ / mZENIQ / etc handling
* `callback.h`
...
4. Enter "zeniq-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "zeniq-qt" under "Run", which is where you can also add command line arguments)
