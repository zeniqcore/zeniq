...
ZeniqNode::ZeniqNode(interfaces::Node &node) : QObject(), m_node(node) {}

void ZeniqNode::handleRunawayException(const std::exception *e) {
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(m_node.getWarnings("gui")));
}

void ZeniqNode::initialize(Config *config, RPCServer *rpcServer,
...
void BitcoinApplication::startThread() {
    ...
    ZeniqNode *executor = new ZeniqNode(m_node);
    executor->moveToThread(coreThread);

    /*  communication to and from thread */
    connect(executor, &ZeniqNode::initializeResult, this,
            &BitcoinApplication::initializeResult);
    connect(executor, &ZeniqNode::shutdownResult, this,
            &BitcoinApplication::shutdownResult);
    connect(executor, &ZeniqNode::runawayException, this,
...
void BitcoinApplication::initializeResult(bool success) {
    ...
    // zeniq: URIs or payment requests:
    if (paymentServer) {
...
void BitcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        ...
        BitcoinGUI::tr("A fatal error occurred. Zeniq can no longer continue "
...
int GuiMain(int argc, char *argv[]) {
    ...
    // Check for -regtest parameter (Params() calls are only valid
    // after this clause)
    try {
    ...
    // Allow for separate UI settings for other networks
    ...
    // zeniq: links repeatedly have their payment requests routed to this
    // process:
