...
Bitcoin-seeder is a crawler for the Zeniq network, which exposes a list
of reliable nodes via a built-in DNS server. It is derived from Pieter Wuille's
zeniq-seeder, modified for use on the Zeniq network.

Features:
* regularly revisits known nodes to check their availability
* bans nodes after enough failures, or bad behaviour
* uses the Zeniq Magic when establishing connections.
...
./zeniq-seeder -h dnsseed.example.com -n vps.example.com
...
Zeniq-seeder is also be used to generate the seed lists that are compiled
into every Zeniq release. It produces the `dnsseed.dump` files that are
