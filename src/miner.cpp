...
BlockAssembler::CreateNewBlock(const CScript &scriptPubKeyIn, double timeLimitSecs, bool checkValidity) {
    ...
    CBlockIndex *pindexPrev = t_NewBlock.ChainActiveTipOrPrevious(chainparams.GetConsensus());
    ...
    if (consensusParams.IsZenitNet() && g_Minter && t_NewBlock.UsedPreviousTip()) {
        t_NewBlock.CopyFromPreviousTip(pblocktemplate.get(), consensusParams);
    } else {
    ...
    if (g_Minter) {
        g_Minter->ToCoinbase(coinbaseTx, nHeight, mempool,
                pindexPrev->GetBlockHash(), pblock, nFees, consensusParams);
    }
    ...
    if (consensusParams.IsZenitNet() && t_NewBlock.counterNoMinter) {
