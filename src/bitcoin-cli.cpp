...
static void SetupCliArgs() {
    ...
    const auto defaultBaseParams = CreateBaseChainParams(CBaseChainParams::ZENIQ);
    ...
    gArgs.AddArg("-rpcport=<port>",
                 strprintf("Connect to JSON-RPC on <port> (default: %u, regtest: %u)",
...
static int AppInitRPC(int argc, char *argv[]) {
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                        "Usage:  zeniq-cli [options] <command> [params]  "
                        "Send command to " PACKAGE_NAME "\n"
                        "or:     zeniq-cli [options] -named <command> "
                        "[name=value]...  Send command to " PACKAGE_NAME
                        " (with named arguments)\n"
                        "or:     zeniq-cli [options] help                "
                        "List commands\n"
                        "or:     zeniq-cli [options] help <command>      Get "
                        "help for a command\n";
    ...
    // Check for -regtest parameter (BaseParams() calls are only
