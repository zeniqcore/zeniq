...
UniValue generateBlocks(const Config &config,
    ...
    static const int nInnerLoopCount = 0x400000;
    int nHeightEnd = 0;
    int nHeight = 0;
...
// NOTE: Unlike wallet RPC (which use ZENIQ values), mining RPCs follow GBT (BIP
...
static UniValue getblocktemplatecommon(bool fLight, const Config &config, const JSONRPCRequest &request) {
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0) {
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED,
                           "Zeniq is not connected!");
    }

    if (IsInitialBlockDownload()) {
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                           "Zeniq is downloading blocks...");
    }
...
void static ZeniqMiner(int ndx) {
    const uint32_t nInnerLoopCount = 0x1000000;
    const uint32_t nInnerLoopStart = ndx * nInnerLoopCount;
    const uint32_t nInnerLoopEnd = nInnerLoopStart + nInnerLoopCount;

    LogPrintf("Zeniq-Miner started\n");
    //SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("Zeniq-Miner");
    ...
    try {
        ...
        while (true) {
            ...
            // Wait to be connected and in sync
            if (IsInitialBlockDownload() ||
                (g_connman->GetNodeCount(
                    CConnman::CONNECTIONS_ALL)) < MINTER_MINING_MIN_NODES) {
                continue;
            }
            ...
            try {
                ...
                const uint64_t startTime = GetSystemTimeInSeconds();
                ...
                if (t_NewBlock.counterNoMinter ||
                    t_NewBlock.TooLowMinterCount(pblock)) {
                    MilliSleep(NOMINTER_SLEEP_MILLI);
                    continue;
                }

                pblock->hashMerkleRoot = BlockMerkleRoot(*pblock);

                pblock->nNonce = nInnerLoopStart;
                while (pblock->nNonce < nInnerLoopEnd &&
                       !CheckProofOfWork(pblock->GetHash(), pblock->nBits,
                           config.GetChainParams().GetConsensus())) {
                    uint32_t nn = ++pblock->nNonce;

                    if (!(nn & 0xFFFF)) {
                        boost::this_thread::interruption_point();

                        const uint64_t now = GetSystemTimeInSeconds();
                        if(now - startTime >= MAX_NONCE_TIME) {
                            // most probably need a new block
                            outOfTime = true;
                            break;
                        }
                    }
                }
                if (outOfTime || pblock->nNonce == nInnerLoopEnd) {
                    continue;
                }
                ...
                LogPrintf("Zeniq-Miner runtime error: %s\n", e.what());
            }
        }
    }
    catch (const boost::thread_interrupted &) {
        LogPrintf("Zeniq-Miner terminated\n");
        throw;
    }
}

void MineZeniq(bool fGenerate, int nThreads) {
