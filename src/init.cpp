...
void Shutdown(NodeContext &node) {
    ...
    if (g_Minter) {
        UnregisterValidationInterface(g_Minter);
        Minter::Exit();
    }
...
void SetupServerArgs() {
    ...
    const auto zeniqBaseParams = CreateBaseChainParams(CBaseChainParams::ZENIQ);
    const auto regtestBaseParams = CreateBaseChainParams(CBaseChainParams::REGTEST);
    const auto zeniqChainParams = CreateChainParams(CBaseChainParams::ZENIQ);
    ...
    gArgs.AddArg(
        ...
        strprintf(
            ...
            "verify all, zeniq: %s)",
            zeniqChainParams->GetConsensus().defaultAssumeValid.GetHex()),
    ...
    gArgs.AddArg("-excessiveblocksize=<n>",
                 strprintf("Do not accept blocks larger than this limit, in "
                           "bytes (zeniq: %u, regtest: %u)",
                           zeniqChainParams->GetConsensus().nDefaultExcessiveBlockSize,
    ...
    gArgs.AddArg("-maxmempool=<n>", strprintf("Keep the transaction memory pool below <n> "
                 "megabytes (zeniq: %u)",
                 DEFAULT_MAX_MEMPOOL_SIZE_PER_MB * zeniqChainParams->GetConsensus().nDefaultExcessiveBlockSize / ONE_MEGABYTE),
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg(
        ...
        strprintf(
            ...
            "(zeniq: %s)",
            zeniqChainParams->GetConsensus().nMinimumChainWork.GetHex()),
        ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
    gArgs.AddArg(
        "-par=<n>",
    ...
    gArgs.AddArg("-port=<port>",
                 strprintf("Listen for connections on <port> (zeniq: %u, regtest: %u)",
                           zeniqChainParams->GetDefaultPort(),
    ...
    gArgs.AddArg(
        ...
        strprintf("Do a full consistency check for mapBlockIndex, "
                  ...
                  "mapBlocksUnlinked occasionally. (zeniq: %u, regtest: %u)",
                  zeniqChainParams->DefaultConsistencyChecks(),
                  regtestChainParams->DefaultConsistencyChecks()),
        ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg(
        "-checkmempool=<n>",
        strprintf(
            "Run checks every <n> transactions (zeniq: %u, regtest: %u)",
            zeniqChainParams->DefaultConsistencyChecks(),
            regtestChainParams->DefaultConsistencyChecks()),
        ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg(
        ...
        strprintf("Activation time for tests (<n> seconds since epoch, "
                  "zeniq: %d, regtest: %d). This option only has an effect on regtest or scalenet.",
                  zeniqChainParams->GetConsensus().axionActivationTime,
                  regtestChainParams->GetConsensus().axionActivationTime),
        ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg(
        "-upgrade8activationtime=<n>",
        strprintf("A future unplanned activation time (<n> seconds since epoch, "
                  "zeniq: %d, regtest: %d)",
                  regtestChainParams->GetConsensus().upgrade8ActivationTime,
                  zeniqChainParams->GetConsensus().upgrade8ActivationTime),
        ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg(
        "-printtoconsole",
    ...
    gArgs.AddArg(
        "-acceptnonstdtxn",
        strprintf(
            "Relay and mine \"non-standard\" transactions (regtest only, default: %d)",
            zeniqChainParams->RequireStandard()),
    ...
    gArgs.AddArg("-blockmaxsize=<n>",
                 strprintf("Set maximum block size in bytes (zeniq: %u, regtest: %u)",
                           zeniqChainParams->GetConsensus().nDefaultGeneratedBlockSize,
...
#ifdef ENABLE_WALLET
    gArgs.AddArg("-gen",
                 strprintf("Start mining (default: 0)"),
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    gArgs.AddArg("-genproclimit",
                 strprintf("Number of thread for mining (default: -1)"),
                 ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#endif
    gArgs.AddArg("-minting=<minting key>[:destination address or wallet name]",
                 strprintf("Set the minting key. "
                           "Optionally provide a destination address or a wallet name,"
#ifdef ENABLE_WALLET
                           "else the first wallet is used."),
#else
                           "else the public key generated from the private minting key is used."),
#endif
                 ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
std::string LicenseInfo() {
    constexpr auto URL_SOURCE_CODE = "<https://github.com/zeniqcore/zeniq>";
    constexpr auto URL_WEBSITE = "<https://zeniq.net>";
...
#ifndef DMA_TESTING
    static const uint8_t dmaData[] =
#include <dmadata.inc>
    ;
#endif
    // DMA_TESTING: assure that not defined for this file by else producing compile error
    if (gArgs.GetArgs("-regtest").empty()) {
        g_Minter = Minter::Create(gArgs.GetArgs("-minting"),dmaData,dmaData+sizeof(dmaData));
        if (!g_Minter) {
            return false;
        }
    }
...
#endif
    if (g_Minter){
        RegisterValidationInterface(g_Minter);
        threadGroup.create_thread(std::bind(&Minter::MinterThread,
                g_Minter));
    }
...
#ifdef ENABLE_WALLET
    // Start mining
    const bool generate = gArgs.GetBoolArg("-gen", false);
    const int genProcs = gArgs.GetArg("-genproclimit", -1);
    MineZeniq(generate, genProcs);
#endif
