...
AcceptToMemoryPoolWorker(const Config &config, CTxMemPool &pool,
    ...
    bool isDMA = tx.IsDMA();
    if (isDMA && g_Minter && !g_Minter->TxValid(tx, state)){
        return false;
    }
...

    {
        ...
        // own.
        if (!isDMA && !CheckSequenceLocks(pool, tx, STANDARD_LOCKTIME_VERIFY_FLAGS,
                                &lp)) {
            return state.DoS(0, false, REJECT_NONSTANDARD, "non-BIP68-final");
        }

        Amount nFees = Amount::zero();
        if (!isDMA && !Consensus::CheckTxInputs(tx, state, view, GetSpendHeight(view),
                                      nFees)) {
        ...
        if (!isDMA && !CheckInputs(tx, state, view, true, scriptVerifyFlags, true, false,
        ...
        if (!isDMA && !CheckInputsFromMempoolAndCache(tx, state, view, pool,
...
// Zeniq: not used (overwritten by SubsidyMiner and SubsidyMinter)
...
bool CChainState::ActivateBestChain(const Config &config,
    ...
    // A node maintains a fair degree of expensive-to-calculate internal state
...
bool CheckBlock(const CBlock &block, CValidationState &state,
    ...
    if (!CheckCoinbase(*block.vtx[0], state, &block.hashPrevBlock, params.IsZenitNet())) {
    ...
    if (params.IsZenitNet() && g_Minter && !g_Minter->CheckMinterCount(state, block)) {
        return false;
    }
...
static bool ContextualCheckBlockHeader(const CChainParams &params,
    ...
    // Check proof of work
    const Consensus::Params &consensusParams = params.GetConsensus();
    uint32_t nRequiredBits = GetNextWorkRequired(pindexPrev, &block, consensusParams);
    if (consensusParams.IsZenitNet() && g_Minter &&
            !IsDarkGravityWaveEnabled(consensusParams,pindexPrev)) {
        arith_uint256 bnBlock;
        bnBlock.SetCompact(block.nBits);
        arith_uint256 bnReq;
        bnReq.SetCompact(nRequiredBits);
        if (bnBlock > bnReq) {
            LogPrintf("bad dark bits after height: %d\n", pindexPrev->nHeight);
            return state.DoS(100, false, REJECT_INVALID, "bad-dark-diffbits", false,
                             "incorrect dark proof of work");
        }
    } else {
    ...
    // Reject outdated version blocks when 95% of the network
    // has upgraded:
    // check for version 2, 3 and 4 upgrades
    // Zeniq block.nVersion in higher word and not !<4. see ZeniqVersion()
    ...
    if (consensusParams.IsZenitNet() && g_Minter &&
            !g_Minter->ContextualCheck(block, state, pindexPrev)) {
        return false;
    }
...
bool CChainState::AcceptBlock(const Config &config,
    ...
    if (isSameHeight && consensusParams.IsZenitNet() && g_Minter &&
            NewBlock::UsePreviousTip(m_chain.Tip())) {
        fHasMoreOrSameWork = true;
        fRequested = true;
    }
 ...
 * take place until the longest chain is at least a certain length
 * Pruning will never delete a block
...
bool CChainState::LoadBlockIndex(const Config &config,
    ...
    for (const std::pair<const BlockHash, CBlockIndex *> &item : mapBlockIndex) {
        ...
        if (pindex->nHeight == 0 && pindex->hashMerkleRoot.IsNull()) {
            // oops! this block index was never loaded from the disk db! Corruption likely.
