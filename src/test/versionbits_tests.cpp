        ...
        for (int i = 0; i < CHECKERS; i++) {
            if (InsecureRandBits(i) == 0) {
                ...
                BOOST_CHECK_MESSAGE(
                    ...
                    strprintf("Test %i for StateSinceHeight (always active)",
                              num));
            }
        }
        num++;
...
BOOST_AUTO_TEST_CASE(versionbits_test) {
    ...
    const auto chainParams = CreateChainParams(CBaseChainParams::ZENIQ);
    const Consensus::Params &zeniqnetParams = chainParams->GetConsensus();
    for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; i++) {
        uint32_t bitmask = VersionBitsMask(
            zeniqnetParams, static_cast<Consensus::DeploymentPos>(i));
        ...
        for (int j = i + 1; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS;
             j++) {
            if (VersionBitsMask(zeniqnetParams,
                                static_cast<Consensus::DeploymentPos>(j)) ==
                bitmask) {
                BOOST_CHECK(zeniqnetParams.vDeployments[j].nStartTime >
                                zeniqnetParams.vDeployments[i].nTimeout ||
                            zeniqnetParams.vDeployments[i].nStartTime >
                                zeniqnetParams.vDeployments[j].nTimeout);
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion) {
    ...
    int64_t bit =
        zeniqnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].bit;
    int64_t nStartTime =
        zeniqnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nStartTime;
    int64_t nTimeout =
        zeniqnetParams.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nTimeout;
    ...
    lastBlock = firstChain
                    .Mine(zeniqnetParams.nMinerConfirmationWindow, nTime,
                          VERSIONBITS_LAST_OLD_BLOCK_VERSION)
                    .Tip();
    BOOST_CHECK_EQUAL(
        ComputeBlockVersion(lastBlock, zeniqnetParams) & (1 << bit), 0);

    // Mine more blocks (4 less than the adjustment period) at the old time, and
    // check that CBV isn't setting the bit yet.
    for (uint32_t i = 1; i < zeniqnetParams.nMinerConfirmationWindow - 4; i++) {
        lastBlock = firstChain
                        .Mine(zeniqnetParams.nMinerConfirmationWindow + i, nTime,
                              VERSIONBITS_LAST_OLD_BLOCK_VERSION)
                        .Tip();
    ...
    nTime = nStartTime;
    for (uint32_t i = zeniqnetParams.nMinerConfirmationWindow - 4;
         i <= zeniqnetParams.nMinerConfirmationWindow; i++) {
        lastBlock = firstChain
                        .Mine(zeniqnetParams.nMinerConfirmationWindow + i, nTime,
                              VERSIONBITS_LAST_OLD_BLOCK_VERSION)
                        .Tip();
    ...
    lastBlock = firstChain
                    .Mine(zeniqnetParams.nMinerConfirmationWindow * 3, nTime,
                          VERSIONBITS_LAST_OLD_BLOCK_VERSION)
                    .Tip();
    // so ComputeBlockVersion should now set the bit,
    BOOST_CHECK((ComputeBlockVersion(lastBlock, zeniqnetParams) & (1 << bit)) !=
                0);
    // and should also be using the VERSIONBITS_TOP_BITS.
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, zeniqnetParams) &
                          VERSIONBITS_TOP_MASK,
                      VERSIONBITS_TOP_BITS);

    // Check that ComputeBlockVersion will set the bit until nTimeout
    nTime += 600;
    uint32_t blocksToMine = zeniqnetParams.nMinerConfirmationWindow *
                            2; // test blocks for up to 2 time periods
    uint32_t nHeight = zeniqnetParams.nMinerConfirmationWindow * 3;
    ...
    for (uint32_t i = 0; i < zeniqnetParams.nMinerConfirmationWindow - 1; i++) {
        lastBlock =
            firstChain
                .Mine(nHeight + 1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION)
                .Tip();
    ...
    lastBlock = secondChain
                    .Mine(zeniqnetParams.nMinerConfirmationWindow * 2, nTime,
    ...
    lastBlock = secondChain
                    .Mine((zeniqnetParams.nMinerConfirmationWindow * 3) - 1,
