...
//    "5ZvhRtjdhcbW3UYXAf9HUzbFMfHW59ii1FsdysgEDT4soJdCJXD";
//static const std::string strSecret1C =
//    "MB1aSuH3sciy9o9akWwMxfWqaSCqPdFewn4aAMR5e2FZRZXXz2Ur";
...
BOOST_AUTO_TEST_CASE(sig_pubkey_hash_variations) {
    ...
    CKey keySecret;
    keySecret.MakeNewKey(false);
    const std::string strSecret1 = EncodeSecret(keySecret);
    CKey keySecretC; // needs to be the same, but compressed
    keySecretC.Set(keySecret.begin(),keySecret.end(),true);
    const std::string strSecret1C = EncodeSecret(keySecretC);
