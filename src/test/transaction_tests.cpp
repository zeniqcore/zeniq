...
BOOST_AUTO_TEST_CASE(tx_valid) {
    ...
    // apply, or "NONE"
    Minter::Exit(); //else CheckCoinbase fails
    ...
    for (const UniValue& test : tests) {
        ...
        if (test[0].isArray()) {
            ...
            bool IsZenitNet = GetConfig().GetChainParams().GetConsensus().IsZenitNet();
            CValidationState state;
            BOOST_CHECK_MESSAGE(tx.IsCoinBase()
                                    ? CheckCoinbase(tx, state, nullptr, IsZenitNet)
...
BOOST_FIXTURE_TEST_CASE(checktxinput_test, TestChain100Setup) {
    ...
    CScript noppyScriptPubKey;
    {
        ...
        //Zeniq: m_coinbase_txns[0->1], as genesisblock does not get DMA transactions
        funding_tx_1.vin[0].prevout = COutPoint(m_coinbase_txns[1]->GetId(), 0);
        funding_tx_1.vout.resize(1);
        funding_tx_1.vout[0].nValue = 1 * COIN; //Zeniq: one coin per minter for the block
        ...
        uint256 fundingSigHash = SignatureHash(p2pk_scriptPubKey, CTransaction(funding_tx_1), 0, SigHashType().withForkId(), m_coinbase_txns[1]->vout[0].nValue);
    ...
    CMutableTransaction funding_tx_2;
    {
        ...
        funding_tx_2.vin[0].prevout = COutPoint(m_coinbase_txns[2]->GetId(), 0);
        funding_tx_2.vout.resize(1);
        funding_tx_2.vout[0].nValue = 1 * COIN;  //Zeniq: one coin per minter for the block

        noppyScriptPubKey << OP_IF << OP_NOP10 << OP_ENDIF << OP_1;
        funding_tx_2.vout[0].scriptPubKey = noppyScriptPubKey;
        std::vector<uint8_t> fundingVchSig;
        uint256 fundingSigHash = SignatureHash(p2pk_scriptPubKey, CTransaction(funding_tx_2), 0, SigHashType().withForkId(), m_coinbase_txns[1]->vout[0].nValue);
        BOOST_CHECK(coinbaseKey.SignECDSA(fundingSigHash, fundingVchSig));
        fundingVchSig.push_back(uint8_t(SIGHASH_ALL | SIGHASH_FORKID));
