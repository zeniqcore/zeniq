...
BOOST_AUTO_TEST_CASE(get_next_work) {
    DummyConfig config(CBaseChainParams::ZENIQ);
...
BOOST_AUTO_TEST_CASE(cash_difficulty_test) {
    ...
    BOOST_CHECK_EQUAL(nBits, 0x1d0fe7b1);
    ...
    // Check the actual value.
    BOOST_CHECK_EQUAL(nBits, 0x1d0db19f);
    ...
    // Check the actual value.
    BOOST_CHECK_EQUAL(nBits, 0x1d0d9222);
    ...
    // Check the actual value.
    BOOST_CHECK_EQUAL(nBits, 0x1e00ffff);
...
BOOST_AUTO_TEST_CASE(asert_difficulty_test) {
    ...
    mutableParams.nASERTHalfLife[0] = 2*24*60*60;
    mutableParams.nASERTHalfLife[1] = 2*24*60*60;
...
BOOST_AUTO_TEST_CASE(calculate_asert_test) {
    DummyConfig config(CBaseChainParams::ZENIQ);
    Consensus::Params mutableparams = config.GetChainParams().GetConsensus();
    mutableparams.nASERTHalfLife[0] = 2*24*60*60;
    mutableparams.nASERTHalfLife[1] = 2*24*60*60;
    mutableparams.powLimit = uint256S("00000000ffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
    const Consensus::Params &params = mutableparams;
    const int64_t nHalfLife = params.nASERTHalfLife[0];
...
BOOST_AUTO_TEST_CASE(asert_activation_anchor_test) {
    // Make a custom chain params based on zeniqnet,
    // activating the cw144 DAA at a random but not so high height
    Consensus::Params params = CreateChainParams(CBaseChainParams::ZENIQ)->GetConsensus();
    params.asertAnchorParams.reset(); // clear hard-coded anchor block so that we may test the activation below
    params.nASERTHalfLife[0] = 2*24*60*60;
    params.nASERTHalfLife[1] = 2*24*60*60;
    ...
    // an arbitrary compact target for our chain
    ...
    BOOST_CHECK_EQUAL(GetNextWorkRequired(pindexPreActivation, &blkHeaderDummy, params), 0x1e00ffff);
    ...
    BOOST_CHECK(IsAxionEnabled(params, indexActivation2.get()));
    BOOST_CHECK_EQUAL(GetNextWorkRequired(indexActivation3.get(), &blkHeaderDummy, params), 0x1d686c79);
