...
public:
    RpcRawSign(){
        dest.MakeNewKey(true);
        priv1.MakeNewKey(true);
        priv2.MakeNewKey(true);
        redeemscript = CScript() << OP_1
            << ToByteVector(priv1.GetPubKey())
            << ToByteVector(priv2.GetPubKey())
    ...
    std::string hexDest(){
        return EncodeDestination(dest.GetPubKey().GetID(),config);
    }
    std::string hexPriv1(){
        return EncodeSecret(priv1);
    }
    std::string hexPriv2(){
        return EncodeSecret(priv2);
    }
    std::string hexRedeem(){
        return HexStr(redeemscript.begin(),redeemscript.end());
    }
    std::string hexScriptPubKey(){
        return HexStr(scriptPubKey.begin(),scriptPubKey.end());
    }
};
...
BOOST_AUTO_TEST_CASE(rpc_submitblock_parallel) {
    ...
    for (auto & thr : threads)
        thr.join();
    // ensure all results match what we expect
