...
BOOST_FIXTURE_TEST_CASE(dsproof_doublespend_mempool, TestChain100Setup) {
    ...
    for (const auto & tx : m_coinbase_txns) {
        ...
        spend.vout.resize(1);
        spend.vout[0].nValue = int64_t(GetRand(1'000)) * CENT / 2;
    ...
    for (size_t i = 0; i < spends.size(); ++i) {
        ...
        spends[i].vout[0].nValue = int64_t(1+i) * CENT / 2;
...
BOOST_FIXTURE_TEST_CASE(dsproof_recursive_search_mempool, TestChain100Setup) {
    ...
    for (size_t i = 0; i < spends.size(); ++i) {
        ...
        spends[i].vout[0].nValue = cbTxRef->vout[0].nValue - int64_t(i+1) * CENT / 2;
        spends[i].vout[0].scriptPubKey = scriptPubKey;
        spends[i].vout[1].nValue = cbTxRef->vout[0].nValue - spends[i].vout[0].nValue;
    ...
    constexpr size_t txChainLen = 17; // longer chains produce !ok2 with reason 'dust' for zeniq
    ...
    for (const auto &ds : dblSpendRoots) {
        ...
        for (size_t i = 0; i < txChainLen; ++i) {
            ...
            tx.vout.resize(2);
            const Amount prevValueOut = parent->vout[0].nValue;
