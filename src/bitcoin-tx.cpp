...
static int AppInitRawTx(int argc, char *argv[], Config &config) {
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage =
            PACKAGE_NAME " zeniq-tx utility version " + FormatFullVersion() +
            "\n\n" +
            "Usage:  zeniq-tx [options] <hex-tx> [commands]  Update "
            "hex-encoded zeniq transaction\n" +
            "or:     zeniq-tx [options] -create [commands]   Create "
            "hex-encoded zeniq transaction\n" +
            "\n";
