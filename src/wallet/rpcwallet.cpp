    ...
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(
            RPCHelpMan{"getnewaddress",
                "\nReturns a new Zeniq address for receiving payments.\n"
                ...
                .ToString() +
            "\nResult:\n"
            "\"address\"    (string) The new Zeniq address\n"
            "\nExamples:\n" +
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Zeniq address for receiving change.\n"
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Zeniq address to be associated with a label."},
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Zeniq address to send to."},
...
static UniValue listaddressgroupings(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The Zeniq address\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Zeniq address to use for the private key."},
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Zeniq address for transactions."},
...
static UniValue sendmany(const Config &config, const JSONRPCRequest &request) {
            ...
            RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, /* opt */ false, /* default_val */ "", "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, /* opt */ false, /* default_val */ "", "The Zeniq address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
    ...
    for (auto &entry : sendTo) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Zeniq address: ") +
                                   entry.first);
        }
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        const std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "Each key is a Zeniq address or hex-encoded public key.\n"
                ...
                "If 'label' is specified (DEPRECATED), assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "A json array of Zeniq addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "Zeniq address or hex-encoded public key"},
                        },
                        },
...
UniValue listtransactions(const Config &config, const JSONRPCRequest &request) {
    ...
    if (request.fHelp || request.params.size() > 4) {
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The Zeniq address of "
            "the transaction.\n"
...
static UniValue gettransaction(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 1 ||
        ...
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The Zeniq "
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspent",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* opt */ true, /* default_val */ "", "A json array of Zeniq addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, /* opt */ true, /* default_val */ "", "Zeniq address"},
                        },
                    },
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the Zeniq address\n"
            ...
            if (auto changeAddressUV = options_obj.locate("changeAddress")) {
                ...
                if (!IsValidDestination(dest)) {
                    throw JSONRPCError(
                        ...
                        "changeAddress must be a valid Zeniq address");
                }
            ...
            RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* opt */ true, /* default_val */ "pool address", "The Zeniq address to receive the change"},
                            {"include_unsafe", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ RPCArg::Default(DEFAULT_INCLUDE_UNSAFE_INPUTS),
                             "Include inputs that are not safe to spend (unconfirmed transactions from outside keys).\n"
                             "Warning: the resulting transaction may become invalid if one of the unsafe inputs "
                             "disappears.\n"
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"getaddressinfo",
                "\nReturn information about the given Zeniq address. Some information requires the address\n"
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Zeniq address to get the information of."},
                }}
                .ToString() +
            "\nResult:\n"
            "{\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, /* opt */ false, /* default_val */ "", "a json array with outputs (key-value pairs)."
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, /* opt */ true, /* default_val */ "", "A key-value pair. The key (string) is the Zeniq address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, /* opt */ true, /* default_val */ "", "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* opt */ true, /* default_val */ "pool address", "The Zeniq address to receive the change"},
                            {"include_unsafe", RPCArg::Type::BOOL, /* opt */ true, /* default_val */ RPCArg::Default(DEFAULT_INCLUDE_UNSAFE_INPUTS),
                             "Include inputs that are not safe to spend (unconfirmed transactions from outside keys).\n"
                             "Warning: the resulting transaction may become invalid if one of the unsafe inputs "
                             "disappears.\n"
