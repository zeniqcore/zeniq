...
BOOST_FIXTURE_TEST_CASE(importwallet_rescan, TestChain100Setup) {
    ...
    // were scanned, and no prior blocks were scanned.
    {
        ...
        BOOST_CHECK_EQUAL(m_coinbase_txns.size(), 104U);
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup) {
    ...
    BOOST_CHECK_EQUAL(100 * COIN, wallet->GetBalance());
    ...
    // Lock both coins. Confirm number of available coins drops to 0.
    {
        ...
        BOOST_CHECK_EQUAL(available.size(), 3U);
    }
...
BOOST_FIXTURE_TEST_CASE(FastTransaction, ListCoinsTestingSetup) {
    ...
    for (uint8_t i=0; i<2; i++) {
        ...
        BOOST_CHECK(wallet->GetBalance() == 100 * COIN);

        // Each AddTx call will spend some coins then mine a block, adding another 1 coins
        AddTx(CRecipient{GetScriptForRawPubKey({}),   50 * COIN, true /* subtract fee */}, coinsel);
        BOOST_CHECK(wallet->GetBalance() == 100 * COIN);
        AddTx(CRecipient{GetScriptForRawPubKey({}),   50 * COIN, true /* subtract fee */}, coinsel);
        BOOST_CHECK(wallet->GetBalance() == 100 * COIN);
        AddTx(CRecipient{GetScriptForRawPubKey({}),  50 * COIN, true /* subtract fee */}, coinsel);
...
BOOST_FIXTURE_TEST_CASE(wallet_bip69, ListCoinsTestingSetup) {
    ...
    // Ensure we have a bunch of small coins
    {
        ...
        std::vector<CRecipient> toMe(100, CRecipient{myScriptPubKey, 1000 * MICROZENIQ, false});
        for (int i = 0; i < 3; ++i) {
    ...
    std::vector<CRecipient> recipients{{
        {scriptPubKeys[1], 3040 * MICROZENIQ, false /* subtract fee */},
        {scriptPubKeys[3], 8001 * MICROZENIQ, false /* subtract fee */},
        {scriptPubKeys[2], 1234 * MICROZENIQ, false /* subtract fee */},
        {scriptPubKeys[0], 234 * MICROZENIQ, false /* subtract fee */},
    }};
