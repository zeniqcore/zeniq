...
bool IsDarkGravityWaveEnabled(const Consensus::Params &params, int nHeight) {
    return nHeight >= params.ZeniqDarkGravityWaveHeight;
}

bool IsDarkGravityWaveEnabled(const Consensus::Params &params,
                  const CBlockIndex *pindexPrev) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsDarkGravityWaveEnabled(params, pindexPrev->nHeight);
}
