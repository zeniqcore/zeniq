...
bool CheckCoinbase(const CTransaction &tx, CValidationState &state,
        const uint256 *hashPrevBlock, bool isZeniqNet) {
    if (!tx.IsCoinBase()) {
    ...
    if (isZeniqNet && g_Minter) {
        bool is_not_genesis = hashPrevBlock != nullptr && *hashPrevBlock != BlockHash();
        if (is_not_genesis &&
            !g_Minter->CheckCoinbase(tx, state, hashPrevBlock)) {
            return false;
        }
        return true;
    }
