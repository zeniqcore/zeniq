...
const char *const BITCOIN_CONF_FILENAME = "zeniq.conf";
const char *const BITCOIN_PID_FILENAME = "zeniq.pid";
...
public:
    ...
    static inline bool UseDefaultSection(const ArgsManager &am,
        ...
        return (am.m_network == CBaseChainParams::ZENIQ ||
...
std::set<std::string> ArgsManager::GetUnsuitableSectionOnlyArgs() const {
    ...
    if (m_network == CBaseChainParams::ZENIQ) {
        return {};
    }
...
bool ArgsManager::ParseParameters(int argc, const char *const argv[],
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; // zeniq-tx using stdin
        std::string val;
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Zeniq
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Zeniq
// Mac: ~/Library/Application Support/Zeniq
// Unix: ~/.zeniq
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zeniq";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Zeniq";
#else
    // Unix
    return pathRet / ".zeniq";
#endif
#endif
}
...
std::string ArgsManager::GetChainName() const {
    LOCK(cs_args);
    if (ArgsManagerHelper::GetNetBoolArg(*this, "-regtest")) {
        return CBaseChainParams::REGTEST;
    }
    return CBaseChainParams::ZENIQ;
}
