...
case "${COMMIT_TYPE}" in
  update-chainparams)
    # Assumes bitcoind instances are already running on zeniqnet
    pushd "${CHAINPARAMS_SCRIPTS_DIR}"
    CHAINPARAMS_ZENIQNET_TXT="chainparams_zeniq.txt"
    ./make_chainparams.py > "${CHAINPARAMS_ZENIQNET_TXT}"
    git add "${CHAINPARAMS_ZENIQNET_TXT}"
