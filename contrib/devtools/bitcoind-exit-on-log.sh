...
DEFAULT_BITCOIND="${TOPLEVEL}/build/src/zeniqd"
DEFAULT_LOG_FILE=~/".bitcoin/debug.log"

help_message() {
  set +x
  echo "Run zeniqd until a given log message is encountered, then kill zeniqd."
  echo ""
  echo "Example usages:"
  ...
  echo ""
  echo "-c, --callback        (optional) Bash command to execute as a callback. This is useful for interacting with zeniqd before it is killed (to run tests, for example)."
  echo "-p, --params          (optional) Parameters to provide to zeniqd."
  echo ""
...
if [ "${BITCOIND_EXIT_CODE}" -ne "0" ]; then
  echo "zeniqd exited unexpectedly with code: ${BITCOIND_EXIT_CODE}"
