...
def get_chainparams(rpc_caller, block):
    ...
    if chaininfo['chain'] == 'zeniq':
        chain = Chain.ZeniqNet
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=(
        ...
        "Prerequisites: RPC access to a zeniqd node.\n\n"),
        formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('--address', '-a', default="127.0.0.1:18581",
                        help="Node address for making RPC calls.\n"
                             "The chain will be automatically detected.\n"
                             "Default: '127.0.0.1:18581'")
    parser.add_argument('--block', '-b',
                        help="The block hash or height to use for fetching chainparams.\n"
                             "Default: 10 blocks from the chain tip.")
    parser.add_argument('--config', '-c', default="~/.zeniq/zeniq.conf",
                        help="Path to zeniq.conf for RPC authentication arguments (rpcuser & rpcpassword).\n"
                             "Default: ~/.zeniq/zeniq.conf")
    ...
    if os.path.isfile(args.config):
        ...
        raise FileNotFoundError("Missing zeniq.conf")
    if user is None:
