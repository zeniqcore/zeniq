...
def build():
    ...
    subprocess.check_call(['make', '-C', '../zeniq/depends',
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'bitcoin=' + args.commit,
                               '--url', 'bitcoin=' + args.url, '../zeniq/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-linux', '--destination', '../gitian.sigs/', '../zeniq/contrib/gitian-descriptors/gitian-linux.yml'])
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'bitcoin=' + args.commit,
                               '--url', 'bitcoin=' + args.url, '../zeniq/contrib/gitian-descriptors/gitian-win.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-win-unsigned', '--destination', '../gitian.sigs/', '../zeniq/contrib/gitian-descriptors/gitian-win.yml'])
    ...
    if args.macos:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'bitcoin=' + args.commit,
                               '--url', 'bitcoin=' + args.url, '../zeniq/contrib/gitian-descriptors/gitian-osx.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version +
                               '-osx-unsigned', '--destination', '../gitian.sigs/', '../zeniq/contrib/gitian-descriptors/gitian-osx.yml'])
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-i', '--commit', 'signature=' + args.commit,
                               '../zeniq/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version + '-win-signed',
                               '--destination', '../gitian.sigs/', '../zeniq/contrib/gitian-descriptors/gitian-win-signer.yml'])
        subprocess.check_call(
    ...
    if args.macos:
        ...
        subprocess.check_call(['bin/gbuild', '-i', '--commit', 'signature=' + args.commit,
                               '../zeniq/contrib/gitian-descriptors/gitian-osx-signer.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version + '-osx-signed',
                               '--destination', '../gitian.sigs/', '../zeniq/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def verify():
    ...
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-linux', '../zeniq/contrib/gitian-descriptors/gitian-linux.yml'])
    print('\nVerifying v' + args.version + ' Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-win-unsigned', '../zeniq/contrib/gitian-descriptors/gitian-win.yml'])
    print('\nVerifying v' + args.version + ' MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-osx-unsigned', '../zeniq/contrib/gitian-descriptors/gitian-osx.yml'])
    print('\nVerifying v' + args.version + ' Signed Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-win-signed', '../zeniq/contrib/gitian-descriptors/gitian-win-signer.yml'])
    print('\nVerifying v' + args.version + ' Signed MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version +
                           '-osx-signed', '../zeniq/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://gitlab.com/zeniq/zeniq.git',
    ...
    # Move to zeniq folder
    os.chdir(os.path.join(workdir, 'zeniq'))
    ...
    # Compare our own source code to zeniq version of gitian-build.py
