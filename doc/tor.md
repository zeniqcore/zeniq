...
It is possible to run Zeniq as a Tor hidden service, and connect to
such services.
...
## 1. Run Zeniq behind a Tor proxy

The first step is running Zeniq behind a Tor proxy. This will already
...
In a typical situation, this suffices to run behind a Tor proxy:
	...
	./zeniqd -proxy=127.0.0.1:9050


## 2. Run a Zeniq hidden server
...
For newer versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServiceDir /var/lib/tor/zeniq-service/
	HiddenServicePort 8333 127.0.0.1:8333
	HiddenServicePort 18333 127.0.0.1:18333

The directory can be different of course, but (both) port numbers should be equal
to your zeniqd's P2P listen port (8333 by default).

	-externalip=X   You can tell Zeniq about its publicly reachable address using
	                this option, and this can be a .onion address. Given the above
	                configuration, you can find your .onion address in
	                /var/lib/tor/zeniq-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./zeniqd -proxy=127.0.0.1:9050 -externalip=57qr3yd1nyntf5k.onion -listen
...
for normal IPv4/IPv6 communication, use:
	...
	./zeniqd -onion=127.0.0.1:9050 -externalip=57qr3yd1nyntf5k.onion -discover

## 3. Automatically listen on Tor

Starting with Tor version 0.2.7.1 it is possible, through Tor's control socket
API, to create and destroy 'ephemeral' hidden services programmatically.
Zeniq has been updated to make use of this.

This means that if Tor is running (and proper authentication has been configured),
Zeniq automatically creates a hidden service to listen on. This will positively
affect the number of available .onion nodes.

This new feature is enabled by default if Zeniq is listening (`-listen`),
...
to be configured. For cookie authentication the user running zeniqd must have
...
the user running Tor and the user running zeniqd to the same group and setting
permissions appropriately. On Debian-based systems the user running zeniqd can
...
- Do not add anything but Zeniq ports to the hidden service created
  in section 2.
