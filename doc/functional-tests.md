...
The [/test/](/test/) directory contains integration tests that test zeniqd
...
- [functional](/test/functional) which test the functionality of
  zeniqd and zeniq-qt by interacting with them through the RPC and P2P
  interfaces.
- [util](/test/util) which tests the zeniq utilities, currently only
  zeniq-tx.
...
The P2P and RPC ports used by the zeniqd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another zeniqd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its zeniqd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other zeniqd processes are running.

On linux, the test_framework will warn if there is another
zeniqd process running when the tests are started.

If there are zombie zeniqd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
zeniqd processes running on the system, so should not be used if any non-test
zeniqd processes are being run.**
...
sure zeniqd processes are stopped as above):
...
- When run on Travis, no logs are output to the console. However, if a test
  fails, the `test_framework.log` and zeniqd `debug.log`s will all be dumped
  to the console to help troubleshooting.

To change the level of logs output to the console, use the `-l` command line
argument.

`test_framework.log` and zeniqd `debug.log`s can be combined into a single
...
call methods that interact with the zeniqd nodes-under-test.

If further introspection of the zeniqd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/zeniqd.pid
gdb /home/example/zeniqd <pid>
```
...
- P2P tests have two threads. One thread handles all network communication
  with the zeniqd(s) being tested in a callback-based event loop; the other
  implements the test logic.
- `P2PConnection` is the class used to connect to a zeniqd.  `P2PInterface`
  contains the higher level logic for processing P2P payloads and connecting to
  the Zeniq node application logic. For custom behaviour, subclass the
...
Basic code to support P2P connectivity to a zeniqd.
