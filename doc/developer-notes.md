...
gradually and you are encouraged to use
...
`zeniqd` is a multi-threaded application, and deadlocks or other
...
$ valgrind -v --leak-check=full src/zeniqd -printtoconsole
```
...
`zeniqd` can be compiled with various "sanitizers" enabled, which add
...
- ThreadRPCServer : Remote procedure call handler, listens on port 18018 for connections and services them.

- ZENIQ-Miner : Generates bitcoins (if wallet is enabled).
...
pay attention to for reviewers of zeniq code.
...
- src/univalue
    - There is no longer a single upstream for `src/univalue`. The sources maintain its
...
would be to revert the upstream fix before applying the updates to
Zeniq's copy of LevelDB. In general you should be wary of any upstream
